...
1. **`monolith_discover()`** — no arguments. Returns the namespace inventory: every namespace, its action count, a one-line description (for in-tree namespaces), sorted categories, and `availability:{available,reason,required_plugin}`. Optional-plugin actions remain registered when their dependency is absent; `ui` and `audio` report CommonUI/Metasound separately in `optional_dependencies` while preserving their base functionality. Inventory rows omit action names by default; `include_action_names=true` restores their `actions` arrays. This response also carries a `guide_hint` pointing back here. Call `monolith_discover("<namespace>")` to get the full action list for one namespace (terse by default: action names + one-line descriptions) — do this before calling any action you have not used. For an action's full **parameter schema**, call `describe_query("action_schema", ...)` (or pass `detail=true` to `monolith_discover` to inline every action's schema), rather than guessing argument names.
...
- **"Unknown namespace" / missing action.** Check spelling, the connected editor/project, loaded Monolith version, and whether the owner module was explicitly disabled. A missing optional plugin no longer removes LogicDriver, ComboGraph, CommonUI or MetaSound registrations. Inspect inventory `availability` and mixed-namespace `optional_dependencies`; unavailable actions return `-32010`, `class:"optional_dep_unavailable"`, and `dep_name`. Install/enable the named plugin and rebuild Monolith. `not_compiled` includes releases built with optional dependencies disabled. Core GameplayAbilities is separate from optional BlueprintAttributes support in GAS.
...
- **Discovery includes unavailable optional actions.** Their schemas remain inspectable; availability describes the compiled implementation. Explicit owner-module settings still take effect after restart and appear as disabled inventory entries. Blueprint Assist auto-layout fallback is documented and returns `formatter_used`; explicitly requesting missing Blueprint Assist returns the optional-dependency error.
