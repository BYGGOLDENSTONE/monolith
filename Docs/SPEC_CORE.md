...
Monolith.uplugin
  ...
  MonolithRuntime       — Runtime GAS widget binding extension/types, four BT tasks and BehaviorTree AIController; PreDefault redirects preserve old editor-module paths. No MCP actions. See COMPLETION_2026_09_06.md for verification status.
  MonolithAudioRuntime  — Runtime sound perception data, component subscription and fire-and-forget hearing helper. Dynamically added components require registration. No MCP actions. See COMPLETION_2026_09_06.md for verification status.
...
>
> **Ballpark, current:** **~1,400+ actions across 25+ in-tree namespaces** (query `monolith_discover()` for the live figure — see [public action-count guidance](../CONTRIBUTING.md#code-structure)).
...
| MonolithReflectionIntel | pipeline | 2 | `pr_review`, `release_readiness`. Read-only composer actions that fan out other registered Monolith actions serially on the game thread and aggregate results into a single payload. `pr_review` bundles `risk_query("get_hotspot_score")` + `risk_query("get_cochange_pairs")` + `decision_query("list_decisions")` + `source_query("audit_module_dep_reality")` + (optional) `blueprint_query("audit_cdo_drift")` per changed file. `release_readiness` bundles `monolith_status()` + `decision_query("list_stale")` + `risk_query("get_release_window_hotspots")` + the sentinel-list audit + CHANGELOG completeness audit described in the [pipeline API reference](API_REFERENCE.md#pipeline_queryrelease_readiness). Both carry `readOnlyHint + idempotentHint`. Phase 4a of Reflection Intelligence — v0.17.0 (`Plugins/Monolith/Docs/specs/SPEC_MonolithReflectionIntel.md` §8). |
...
editor-process lease. Acquisition TTL defaults to 120 seconds, bounded to 10..600.
Renewal retains the current TTL when `ttl_seconds` is omitted. Acquisition
returns `_lease_token`; every protected domain request supplies it in params.
Competing calls fail before execution with `-32020`; stale tokens fail with
`-32021`, including after release/restart. An active synchronous handler may finish
past expiry; when the last active leased execution ends, the owner receives a grace period of
`min(30, TTL/4)` seconds to renew or release. Requests after that expiry are fenced.
Nested synchronous registry calls inherit ownership; separate HTTP dispatches explicitly do not. No rollback,
