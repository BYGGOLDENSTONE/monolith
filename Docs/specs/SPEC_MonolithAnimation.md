...
| `add_compatible_skeleton` | Optional `save` (boolean, default false): changes stay dirty in memory unless `save=true`. Marks a target skeleton compatible with the source. Idempotent: returns disjoint `added` / `already_compatible` booleans + resulting `count`. Self-compat rejected with `"Cannot mark a skeleton compatible with itself"`. |
| `remove_compatible_skeleton` | Optional `save` (boolean, default false): changes stay dirty in memory unless `save=true`. Removes a previously-registered compatible skeleton. Idempotent: returns disjoint `removed` / `was_compatible` booleans + resulting `count`. |
...
| Action | Description |
|--------|-------------|
| `add_anim_layer_graph` | Create an ABP-NATIVE animation layer: a `UAnimationGraph` carrying `UAnimationGraphSchema` in the Animation Blueprint's own `FunctionGraphs` — exactly what the editor's My Blueprint → **+** → Animation Layer button produces. The anim schema is what makes the anim compiler emit a real `FAnimBlueprintFunction` for the graph (`blueprint add_function` produces an inert K2 graph instead), and the Output Pose root node is created automatically. Params: `asset_path`, `layer_name`, optional `input_poses` (`["InPose"]` or `[{"name":"InPose","inputs":[{"name":"BlendWeight","type":"float"}]}]`, capped at 16 poses and 32 typed inputs per pose; pose names unique across the ABP, parameter names unique across the layer), optional `compile` (default `true`). Returns `asset_path`, `graph_name`, `graph_class`, `schema_class`, `node_count`, `input_poses`, `parameter_count`, `layer_kind:"native"`, `compiled`, `saved`. Refuses: a duplicate graph name (it never renames or replaces an incumbent graph), the reserved name `AnimGraph`, a child Animation Blueprint (layers belong to the root ABP), macro libraries, interface Blueprints, and duplicate/empty pose names or a non-array `input_poses`. Populate the layer with the anim-graph authoring actions above, then place a consumer node with `add_linked_anim_layer`, which auto-detects native layers. |

**How a native layer reads back.** It reports in `blueprint list_graphs` as `type: "function"` (so does the stock `AnimGraph` — classification is by which array the graph sits in, not by schema), never appears in `blueprint get_interfaces`, and `animation get_linked_layers` shows the consumer node title `"<Layer>\nAnim Layer (self)"`. A self bind is discriminated in the `add_linked_anim_layer` payload by `layer_kind:"native"`, `interface_class: "<self>"` and `guid_resolved: false`; interface-backed calls return `layer_kind:"interface"`. `add_anim_layer_graph`'s `graph_class` / `schema_class` are the machine-checkable proof that the graph is a `UAnimationGraph` on the anim schema rather than a K2 graph — no other action reports either field.

> **Compile and pin refresh (2026-09-06).** Typed inputs are authored through `UAnimGraphNode_LinkedInputPose::Inputs` using strict pin-type parsing. Native input nodes and linked-layer consumers reconstruct their pins; `add_linked_anim_layer` exposes the engine-discovered typed input parameters (normally hidden by UE optional-pin settings) and performs a second reconstruction after compilation makes the signature available. `add_anim_layer_graph(compile=false)` leaves an unfinished graph for subsequent authoring. The default `compile=true` remains convenient for complete layer creation.
...
| `build_state_machine` | Declarative builder composing state/transition/rule creation in one transaction from `{states:[{name, animation?}], transitions:[{from,to,rule?}], entry_state}`. States optionally wire a `UAnimGraphNode_SequencePlayer` to the state result pose pin. Rules support bool variables, `auto`/`automatic`, and structured numeric comparisons. Compound expressions and unusable rule operands are reported through per-transition `rule_deferred` messages while the created graph is retained. Returns per-element reports plus top-level `partial:true, deferred_rules:N` when any rules were deferred; otherwise `partial:false, deferred_rules:0`. Compiles once and leaves the package dirty without saving. |
...
### State-machine rule parity (2026-09-06)

`build_state_machine` supports the same structured numeric `expression` terms and `combine:"and"|"or"` operators as `set_transition_rule`, using the shared expression authoring helper. Invalid operands/grammar and failed compare authoring are reported per transition as `rule_deferred` and counted in the top-level `partial` / `deferred_rules` result. Valid states and transitions remain available for later repair. Arbitrary freeform expression text is not part of the supported grammar.
