...
| PoseSearch | 13 | `get_pose_search_schema`, `get_pose_search_database`, `add_database_sequence`, `remove_database_sequence`, `get_database_stats` (**never blocks since 2026-07-24** — see below), `create_pose_search_schema`, `create_pose_search_database`, `set_database_sequence_properties`, `add_schema_channel`, `remove_schema_channel`, `set_channel_weight`, `rebuild_pose_search_index` (**async by default since 2026-07-24** — returns a `job_id`, poll with `jobs_query`; `wait: true` restores the old blocking call), `set_database_search_mode` |

**PoseSearch index reads never block (2026-07-24).** `get_database_stats` and `validate_pose_search_database` used to wait for an in-flight search-index build to finish, freezing the editor *and* the whole MCP server for minutes. They are reads, so they were not converted into jobs; instead they answer immediately and say what they could not determine. Both now return `index_status` (`built` / `building` / `failed` / `no_schema` / `unavailable`), `index_built`, `index_build_in_progress` (**the field to branch on** — the answer is incomplete), `index_build_job_id` (the Monolith job to poll with `jobs_query`, or `null` when the build was not dispatched through `rebuild_pose_search_index` and therefore cannot be honestly named), `waited`, and a human-readable `index_note`. `get_database_stats` reports `total_pose_count: null` / `is_valid: false` rather than guessing; `validate_pose_search_database` adds `validation_complete: false`, which separates "no problems found" from "did not finish looking" — an in-flight build is listed in `issues` but does **not** make `valid` false. Pass `wait: true` (same spelling and default as `rebuild_pose_search_index`) for the old blocking behaviour. Full contract: `Docs/specs/SPEC_MonolithAnimation.md`.
...
| Atmosphere / GI | `spawn_atmosphere` (`post_process` / `height_fog` / `sky_atmosphere` / `volumetric_cloud`), `set_atmosphere_properties`, `get_atmosphere_properties`, `list_atmosphere_presets`, `get_lumen_settings`, `set_lumen_settings` |
