...
bool FMonolithFuzzyDiscoveryTest::RunTest(const FString& Parameters)
{
	...
	auto CheckSuggestions = [this](const FMonolithActionResult& Result, const TCHAR* Kind, const TCHAR* Expected, int32 Code)
	{
		...
		TestEqual(TEXT("Typed lookup error code"), Result.ErrorCode, Code);
		TArray<FString> Keys;
		TArray<float> Scores;
		if (!TestTrue(TEXT("Error carries structured suggestions"), GetSuggestions(Result, Keys, Scores))) return;
		TestEqual(TEXT("Lookup class is not_found"), Result.ErrorData->AsObject()->GetStringField(TEXT("class")), FString(TEXT("not_found")));
		TestFalse(TEXT("Lookup rejection executed no mutation"), Result.ErrorData->AsObject()->GetBoolField(TEXT("executed")));
	...
	CheckSuggestions(Registry.ExecuteAction(TEXT("monolith"), TEXT("discover"), Params),
		TEXT("namespace"), TEXT("monolith"), FMonolithJsonUtils::ErrNotFound);
	...
	CheckSuggestions(Registry.ExecuteAction(TEXT("describe"), TEXT("action_schema"), Params),
		TEXT("action"), TEXT("discover"), FMonolithJsonUtils::ErrNotFound);
