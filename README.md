...
One plugin. One MCP endpoint. A handful of namespace-dispatch tools instead of ~1,400+. The AI calls `monolith_discover()` and `monolith_guide()` when it needs to know what's available, and otherwise just hits `blueprint_query("create_blueprint", ...)`, `material_query("compile", ...)`, and so on. Discovery starts with namespace counts and categories. Listing a namespace returns action names and one-line descriptions; the full param schema is one `describe_query("action_schema", ...)` call (or `detail=true`) away when the AI needs it.
...
Covered domains: Blueprints, Materials, Animation (including AnimGraph and Motion Matching authoring), Niagara (including direct HLSL editing), Mesh, UI (incl. CommonUI), AI (Behavior Trees, State Trees, EQS, Smart Objects, Perception, Navigation), Gameplay Ability System, Logic Driver state machines, ComboGraph combo trees, Audio (Sound Cues + MetaSounds), Editor control (UBT builds, log capture, scene capture, asset preview & inspection, live-PIE introspection and driving), Engine source search (~967K symbols in the measured index, fully offline, with an LLM C++ authoring pack: include path, signature, deprecation, Build.cs deps, header lint, class stubs), Project asset search (SQLite FTS5), INI config, Level Sequences, a `bulk_fill` / `describe` reflection framework for deep property writes, a `monolith_guide` self-onboarding tool for your AI, plus the **Reflection Intelligence** layer: `decision` (architectural decision-record harvest), `risk` (repo-level hotspot + co-change + conditional-gate signals), `cppreflect` (UE 5.7 UHT reflection-edge queries cross-joined with the asset registry), `network` (replication inspection — replicated classes, RPCs, OnRep handlers, unbalanced-handler audits), `pipeline` (read-only composer actions for PR review + release pre-flight), and `reflect` (index maintenance — a project-only force-rebuild of the reflection tables). The `cppreflect` and `network` indexers scan your project plugins by default, so replicated classes and RPCs declared in plugins are in scope without extra setup; enabled marketplace plugins are gated behind a setting, and Epic engine built-ins stay excluded.

**MCP LLM Ergonomics:** universal response shaping (`_fields` / `_omit` / `_compact_json`) on every action, schema-tagged param kinds with automatic `\` → `/` rewrite on asset paths, `did_you_mean` fuzzy match on dispatch and discovery errors, MCP `tools/list` annotations (read-only / destructive / idempotent hints), `structuredContent` on every tool result, `source_query` cursor pagination, and a proxy-side JSONL call log. The whole point is to let your AI spend less context recovering from typos and trial-and-error.

**Multi-agent:** bounded concurrent proxies and a cooperative editor lease (`monolith_coordination`) let several AI clients share one editor. See [multi-agent setup](Docs/MULTI_AGENT.md).

See the [changelog](CHANGELOG.md) for release history, feature additions and fixes.
