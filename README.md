...
Covered domains include Blueprints, materials, animation, Niagara, meshes, UI and CommonUI, AI, Gameplay Ability System, Logic Driver, ComboGraph, audio, editor control, INI configuration and Level Sequences. Offline engine-source search covers ~967K symbols in the measured index; project asset search uses SQLite FTS5. Reflection tools inspect C++ declarations and replication, collect decision records and repository risk signals, and compose review reports. `bulk_fill` / `describe` supports reflected property access, while `monolith_guide` provides workflow guidance.
