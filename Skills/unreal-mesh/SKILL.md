---
name: unreal-mesh
description: Use Monolith MCP for mesh assets, spatial inspection, level blockouts, actors, collision, and procedural geometry. Applies when working in an Unreal project with Monolith available.
---

# unreal-mesh

Read [the domain reference](unreal-mesh.md) only for the requested operation. Its static action tables are examples; installed tools and per-action schemas are authoritative.

1. Call `monolith_status` to identify the connected project and engine. Discover `mesh` with `monolith_discover({"namespace":"mesh"})`.
2. Resolve the intended action and parameters with `describe_query({"action":"action_schema","params":{"target_namespace":"mesh","target_action":"ACTION"}})`. Use returned asset paths and IDs; do not invent signatures or treat a missing optional namespace as a transport failure.
...
Inspect generated geometry in the scene, collision and navigation, scale, UVs, and measured runtime cost. Experimental town generation is not production-validated geometry.
