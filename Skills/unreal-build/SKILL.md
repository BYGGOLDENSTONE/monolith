---
name: unreal-build
description: Use Monolith MCP for Unreal project compilation, Live Coding, and structural plugin rebuilds. Applies when working in an Unreal project with Monolith available.
---

# unreal-build

Read [the domain reference](unreal-build.md) only for the requested operation. Its static action tables are examples; installed tools and per-action schemas are authoritative.
...
Determine the actual engine installation, project target, editor process, and compile state. An MCP timeout does not mean the editor is closed.
