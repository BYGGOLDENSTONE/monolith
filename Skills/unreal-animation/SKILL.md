...
description: Use Monolith MCP for animation sequences, montages, Animation Blueprints, skeletons, IK, and retargeting. Applies when working in an Unreal project with Monolith available.
---

# unreal-animation

Read [the domain reference](unreal-animation.md) only for the requested operation. Its static action tables are examples; installed tools and per-action schemas are authoritative.

1. Call `monolith_status` to identify the connected project and engine. Discover `animation` with `monolith_discover({"namespace":"animation"})`.
2. Resolve the intended action and parameters with `describe_query({"action":"action_schema","params":{"target_namespace":"animation","target_action":"ACTION"}})`. Use returned asset paths and IDs; do not invent signatures or treat a missing optional namespace as a transport failure.
...
Validate skeleton compatibility and compiled animation graphs; preview transitions and root motion in context.
