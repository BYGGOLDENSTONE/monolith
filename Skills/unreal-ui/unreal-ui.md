...
**UI actions** are exposed through `ui_query()`. Discover the installed surface with `monolith_discover({ namespace: "ui" })`; filter CommonUI with `monolith_discover({ namespace: "ui", category: "CommonUI" })`. Always-on features include widget CRUD, layout, templates, styling, animation, bindings, settings scaffolds, accessibility, design import, EffectSurface, spec building and type diagnostics. Prefer `create_animation_v2` over the deprecated v1 animation authoring actions.

**CommonUI actions require the CommonUI engine plugin** (stock UE 5.7, `Engine/Plugins/Runtime/CommonUI/`). When absent, the CommonUI registrations are omitted. Detect availability via `monolith_discover`.
...
### E: Lists / Tabs / Groups / Switchers / Carousel / HW Visibility
...
- CommonUI runtime actions require a PIE session for functional testing — M0.5.1 test-coverage pass pending.
