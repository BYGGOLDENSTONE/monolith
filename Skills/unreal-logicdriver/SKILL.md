...
description: Use Monolith MCP for Logic Driver Pro state machines, transitions, and runtime inspection. Applies when working in an Unreal project with Monolith available.
---

# unreal-logicdriver

Read [the domain reference](unreal-logicdriver.md) only for the requested operation. Its static action tables are examples; installed tools and per-action schemas are authoritative.

1. Call `monolith_status` to identify the connected project and engine. Discover `logicdriver` with `monolith_discover({"namespace":"logicdriver"})`.
2. Resolve the intended action and parameters with `describe_query({"action":"action_schema","params":{"target_namespace":"logicdriver","target_action":"ACTION"}})`. Use returned asset paths and IDs; do not invent signatures or treat a missing optional namespace as a transport failure.
...
Check the optional Logic Driver Pro plugin is available. Inspect stub/partial results and verify runtime transitions; discoverable actions are not proof of implementation.
