---
name: unreal-gas
description: Use Monolith MCP for Gameplay Ability System abilities, effects, attributes, tags, and cues. Applies when working in an Unreal project with Monolith available.
---

# unreal-gas

Read [the domain reference](unreal-gas.md) only for the requested operation. Its static action tables are examples; installed tools and per-action schemas are authoritative.

1. Call `monolith_status` to identify the connected project and engine. Discover `gas` with `monolith_discover({"namespace":"gas"})`.
2. Resolve the intended action and parameters with `describe_query({"action":"action_schema","params":{"target_namespace":"gas","target_action":"ACTION"}})`. Use returned asset paths and IDs; do not invent signatures or treat a missing optional namespace as a transport failure.
...
Compile generated classes and Blueprints; exercise activation, cancellation, costs, cooldowns, and replication when applicable. Generated ability tasks can contain TODO bodies.
