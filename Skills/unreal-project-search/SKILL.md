...
description: Use Monolith MCP for Unreal project assets, references, dependencies, and indexed content search. Applies when working in an Unreal project with Monolith available.
---

# unreal-project-search

Read [the domain reference](unreal-project-search.md) only for the requested operation. Its static action tables are examples; installed tools and per-action schemas are authoritative.

1. Call `monolith_status` to identify the connected project and engine. Discover `project` with `monolith_discover({"namespace":"project"})`.
2. Resolve the intended action and parameters with `describe_query({"action":"action_schema","params":{"target_namespace":"project","target_action":"ACTION"}})`. Use returned asset paths and IDs; do not invent signatures or treat a missing optional namespace as a transport failure.
...
Check index freshness and scope; confirm live asset state before editing. A missing indexed result is not proof an asset does not exist.
