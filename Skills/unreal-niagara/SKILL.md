---
name: unreal-niagara
description: Use Monolith MCP for Niagara systems, emitters, modules, renderers, and particle HLSL. Applies when working in an Unreal project with Monolith available.
---

# unreal-niagara

Read [the domain reference](unreal-niagara.md) only for the requested operation. Its static action tables are examples; installed tools and per-action schemas are authoritative.

1. Call `monolith_status` to identify the connected project and engine. Discover `niagara` with `monolith_discover({"namespace":"niagara"})`.
2. Resolve the intended action and parameters with `describe_query({"action":"action_schema","params":{"target_namespace":"niagara","target_action":"ACTION"}})`. Use returned asset paths and IDs; do not invent signatures or treat a missing optional namespace as a transport failure.
...
Compile effects and observe representative lifetime, bounds, scalability, and runtime cost on the target RHI.
